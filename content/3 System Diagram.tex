...
The human metabolic system is a multi-input, single-output system: several inputs act simultaneously on a single output, the measured body mass m. Height, age, and sex (h, a, σ) are fixed parameters, whereas activity (τ, MET), total energy intake (TEI), and sleep duration (s) are controllable inputs, each entering the Energy Balance through its own path. Driving m toward the healthy body mass m_des therefore requires regulating all controllable inputs together; an input left uncontrolled acts as an unregulated disturbance on D that the remaining inputs cannot reliably compensate for, and the output drifts from its desire.
...
\node[block, below=of rolling] (bmr)   {Basal Metabolic Rate\\ \textcolor{gray}{$BMR$}};
\node[block, below=of bmr]     (act)   {Activity Energy Expenditure\\ \textcolor{gray}{$AEE$}};
\node[block, below=of act]     (tef)   {Thermic Effect of Food\\ \textcolor{gray}{$TEF$}};
\node[block, below=of tef]     (sleep) {Sleep Deprivation Effect\\ \textcolor{gray}{$SD$}};

\node[block, minimum height=24mm, right=12mm of $(act.east)!0.5!(tef.east)$, yshift=-4mm] (balance)
  {Energy Balance\\ \textcolor{gray}{$D = TEI - (BMR + AEE + TEF)$}};
...
\draw[signal] (in-tef)   -- node[lbl, above, pos=0.35] {$TEI$}          (tef.west);
...
\draw[signal] ([yshift=-3mm]act.east) -- node[lbl, above] {$AEE$} ([yshift=-3mm]act.east -| balance.west);
\draw[signal] (tef.east) -- node[lbl, above] {$TEF$}   (tef.east -| balance.west);
\draw[signal] (sleep.east) -| node[lbl, above, pos=0.25] {$SD$} ([xshift=8mm]balance.south);
\coordinate (ein-branch) at ($(tef.west)+(-4mm,0)$);
\node[branch] at (ein-branch) {};
\draw[signal] (ein-branch) |- node[lbl, above, pos=0.7] {$TEI$} ($(tef.south)!0.5!(sleep.north)$)
...
\end{tikzpicture}
\caption[Block diagram of the human metabolic system model.]{Block diagram of the human metabolic system model; the shaded region marks the system boundary. External inputs enter from the left: height, age, and sex ($h, a, \sigma$), activity ($\tau$, $MET$), total energy intake ($TEI$), sleep duration ($s$), and elapsed time ($t$). The Basal Metabolic Rate, Activity Energy Expenditure, Thermic Effect of Food, and Sleep Deprivation Effect blocks feed the Energy Balance block, whose daily energy deficit $D$ drives the Body Mass Change block. The Glycogen + Water Swing $\Delta M_{gly}$ is added to that body mass to give the measured body mass $m$, which is fed back through the 7-Day Rolling Average as $\bar{m}$ to the Basal Metabolic Rate, Activity Energy Expenditure, and Glycogen + Water Swing blocks.}
