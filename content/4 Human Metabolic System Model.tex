...
% \bibliographystyle/\bibliography pointing at "8 References.bib".
...
The healthy body mass m_des can equivalently be expressed as a healthy body mass index, BMI_des, computed from m_des and height h. A BMI between 18.5 and 24.9 is the World Health Organization's healthy-weight band \cite{ref_who1995}, and this figure is flagged when it falls outside that range. The relationship between m_des and BMI_des is given in eqn.~\eqref{eqn_bmig}.

\begin{equation}
\label{eqn_bmig}
BMI_{des} = \frac{m_{des}}{(h/100)^{2}}
\end{equation}
...
\subsubsection{Basal Metabolic Rate}

The Katch-McArdle equation predicts basal metabolic rate directly from lean body mass rather than from age and sex, reflecting that metabolically active tissue — not fat mass — drives basal metabolic rate \cite{ref_katch1996}, as given in eqn.~\eqref{eqn_bmrk}.
...
The Mifflin-St Jeor equation instead predicts basal metabolic rate from body mass, height, age, and sex, and was derived by regression on indirect-calorimetry measurements from 498 healthy adults \cite{ref_mifflin1990}, as given in eqn.~\eqref{eqn_bmrm}; it serves as the model's default basal-metabolic-rate formula.
...
The desire daily intake E_in is the calorie level that, net of the thermic effect of food, still leaves the desired deficit D on top of maintenance (BMR + AEE). Because TEF is itself a share of E_in rather than a fixed amount, the identity is solved for E_in directly — placing (1 − f) in the denominator — instead of treating TEF as a further subtraction, as given in eqn.~\eqref{eqn_eink}.

\begin{equation}
\label{eqn_eink}
E_{in} = BMR + AEE + TEF − D = \frac{BMR + AEE − D}{1 − f}
\end{equation}

\subsection{Maintenance and Mass Trajectory}

Maintenance energy expenditure M(m̄) = A + B×m̄ is affine in body mass under either basal-metabolic-rate formula, with the coefficients A and B given in eqns.~\eqref{eqn_amif}–\eqref{eqn_bkat}. Under Katch-McArdle, c\_m, c\_h, and c\_0 are the same sex-specific Boer coefficients from eqn.~\eqref{eqn_lbmm}/\eqref{eqn_lbmf} (0.407, 0.267, −19.2 for males; 0.252, 0.473, −48.3 for females), substituted in place of LBM so A and B stay linear in h alone. Holding intake E_in fixed then drives body mass toward a single equilibrium m∞ = (E_in − A)/B, as given in eqn.~\eqref{eqn_meqm}, and mass approaches that equilibrium exponentially over the elapsed time t (in days) rather than linearly — the same first-order dynamics used to model human body-weight change under sustained energy imbalance \cite{ref_chow2008} — as given in eqn.~\eqref{eqn_mdec}, with t to a healthy mass following from inverting that decay in eqn.~\eqref{eqn_tday}.
...
\begin{equation}
\label{eqn_tday}
t = \frac{ρ}{B} × \ln\!\left[ \frac{m̄ − m∞}{m_{des} − m∞} \right]
\end{equation}
\begin{equation}
\label{eqn_mprp}
...
\begin{equation}
\label{eqn_tprp}
t = \frac{7 × \ln(m̄ / m_{d})}{−\ln(1 − Δm\%/100)}
\end{equation}
\begin{equation}
\label{eqn_ladp}
